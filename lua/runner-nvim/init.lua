...
function M.Runnercode()
  ...
  if vim.g.toggleMakefile and get_name and get_name.Makefile then
    ...
    if get_name and get_name.cmd then
      local get_cmd = source.changeCmd(
        "cmd",
        current_filetype,
        current_withoutext,
        realPath,
...
M.Getdebug = function()
  ...
  if get_name and get_name.debug then
    ...
    show_info_message("Compile debugging mode: " .. current_filename)
    -- show_info_message(Get_cmd)
  else
    return nil
  end
  return Get_cmd
